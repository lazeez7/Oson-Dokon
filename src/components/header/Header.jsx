import React from "react";
import logo from './Снимок_экрана_2024-05-29_184611-removebg-preview.png'
import "./header.css";

const Header = () => {
  return (
    <>
      <div className="container">
        <div className="wrap">
          <div className="card">
            {/* <img className="imgg"
              src="data:image/jpeg;base64,"
              alt=""
            /> */}

            <img src={logo} alt="" />
            {/* <div>
              <a href="">BuddyBurger</a>
              <p>мвмвмвх</p>
            </div> */}
          </div>
          <div className="card">
            <h3>
              <a href="№">Доставка пиццы за 35 мин.</a>
            </h3>
            <p>Работаем круглосуточно 24/7</p>
          </div>
          <div className="card">
            <a href="tel:900510882">
            <img src="https://w7.pngwing.com/pngs/129/394/png-transparent-green-phone-icon-iphone-3g-telephone-call-computer-icons-phone-icon-miscellaneous-text-grass-thumbnail.png" alt="" />
              
            </a>

            <div>
              <a href="tel:712445445">(71) 244 54 45</a>
              <p>Звоните сейчас!</p>
            </div>
          </div>
          <div className="card"> 
            <img className="halal"
              src="https://apexpizza.uz/static/media/sertifikat-xalol.f31879d9808fe10553f5.png"
              alt=""
            />
            <div>
              <a href="№">СЕРТИФИКАТ ХАЛЯЛЪ</a>
              <p>Без свинины</p>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export default Header;
